...
import type { MediaMatcher } from "@/cdk/layout/media-matcher.ts";
import { forwardRef } from "@/core/di/forward-ref.ts";
import { Injectable } from "@/core/di/injectable.ts";
import type { NgZone } from "@/core/platform/ng-zone.ts";
 ...
 * → la vista se refresca sola). Se provee solo en la raíz, como en el CDK.
 */
@Injectable({ providedIn: "root", useClass: forwardRef(() => BreakpointObserverImpl) })
