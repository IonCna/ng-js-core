...
export interface TranslatedRoutes {
  ...
  /** `Route.providers` por state name — se registran en la app (AngularJS tiene un solo injector). */
...
function componentName(route: Route): string | undefined {
  ...
  if (!tag) {
    throw new Error(
      `RouterModule: el component de la ruta "${route.path ?? ""}" no es un @Component compilado con selector de elemento.`,
    );
  }
...
export function registerRouteProviders(routeProviders: Map<string, Provider[]>): void {
...
function lazyLoadFor(route: Route, stateName: string, url: string, data: Data) {
  ...
  return async (context: LazyLoadContext) => {
    ...
    const cls = ((loaded as { default?: unknown }).default ?? loaded) as Function;
    if (!CompiledType.isComponent(cls)) {
      throw new Error(`RouterModule: loadComponent de "${route.path ?? ""}" no resolvió una clase @Component.`);
    }
    // Un componente de `loadComponent` no está en ningún `@NgModule` cargado: se registra al vuelo.
    // Sin módulo propio hereda el `controllerAs` del `@NgModule` raíz, como el componente eager que reemplaza.
    const $injector = context.$injector as angular.auto.IInjectorService;
    const name = ComponentRegistrar.ensure(cls, $injector, ComponentRegistrar.rootControllerAs($injector));
...
function unwrapLazyRoutes(loaded: LoadChildrenResult, $injector: unknown, stateName: string): Routes {
  ...
  if (typeof loaded === "function" && (loaded as { ɵmod?: unknown }).ɵmod) {
...
function lazyLoadChildrenFor(
  ...
  return async (context: LazyLoadContext) => {
    ...
    // Componentes del chunk lazy que no vinieron declarados en un `@NgModule` (`Routes` sueltas) — idempotente.
    for (const { cls } of sub.components) ComponentRegistrar.ensure(cls, $injector as angular.auto.IInjectorService);
