...
class LazyUrlLink {
  ...
  private href(): string {
    if (!this.$injector.has(LocationStrategy.$name)) return this.url;
    return this.$injector.get<LocationStrategy>(LocationStrategy.$name).prepareExternalUrl(this.url);
  }
  ...
  private onClick(event: JQueryEventObject): void {
    ...
    void this.$injector.get<Router>(Router.$name).navigateByUrl(this.url);
  }
