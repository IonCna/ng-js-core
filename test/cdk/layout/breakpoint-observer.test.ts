...
import { type BreakpointObserver, BreakpointObserverImpl, Breakpoints, MediaMatcherImpl } from "@/cdk/layout/index.ts";
import { NgZoneImpl } from "@/core/platform/ng-zone.ts";
...
/** La lógica del servicio; que se provee solo en la raíz lo cubre `cdk.compiled.test.ts`. */
function boot(): BreakpointObserver {
  const $rootScope = angular.injector(["ng"]).get<angular.IRootScopeService>("$rootScope");
  return new BreakpointObserverImpl(new MediaMatcherImpl(), new NgZoneImpl($rootScope));
}
