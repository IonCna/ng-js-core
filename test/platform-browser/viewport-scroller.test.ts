...
import { BrowserViewportScroller, type ViewportScroller } from "@/common/index.ts";

let scrollToSpy: ReturnType<typeof vi.fn>;
let originalScrollTo: typeof window.scrollTo;
let restoration: string;

/** La lógica del servicio; su provisión por `CommonModule` la cubre `platform-browser.compiled.test.ts`. */
function boot(): ViewportScroller {
  return new BrowserViewportScroller(window, document);
}
