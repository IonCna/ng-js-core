...
describe("etapa 18 — LOCALE_ID", () => {
  it("es un InjectionToken con descripción estable y un factory propio (se provee solo en la raíz)", () => {
    expect(String(LOCALE_ID)).toBe("LOCALE_ID");
    expect(typeof LOCALE_ID.options?.factory).toBe("function");
  });
});
