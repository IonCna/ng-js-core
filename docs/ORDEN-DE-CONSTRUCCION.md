...
**Brecha documentada:** árbol de `ActivatedRoute` (`.parent`/`.children`/params-por-nivel), redirect con `UrlTree` desde guard, resolvers `Type<T>`, `Route.providers`, navegación relativa (`relativeTo`); `loadChildren` que devuelve una clase `@NgModule` (forma vieja de Angular); redirect en un subárbol lazy hacia un hermano eager. **`canMatch`**: el "fallthrough" (si da `false`, probá la siguiente ruta / caé al `**`) no se replica — UI-Router matchea por URL de forma única; acá `false` aborta la transición. **`canDeactivate`**: la firma completa está (`(component, currentRoute, currentState, nextState)`), pero `currentState`/`nextState` son `{ url, root }` planos — sin árbol `.children`/params-por-nivel. **`TitleStrategy`**: versión básica — el router resuelve el título y el strategy solo lo aplica (`updateTitle(title)`). Sin snapshot, sin `buildTitle`/`getResolvedTitleForRoute`, sin override de la resolución. **`withInMemoryScrolling`**: solo `anchorScrolling` + `scrollPositionRestoration: 'top'`. `'enabled'` (guardar/restaurar la posición de scroll en back/forward) NO se implementa — UI-Router no expone el trigger de navegación (`popstate` vs imperativa) ni un id por navegación. Tampoco se emite el evento `Scroll` en `Router.events` (se usa `NavigationEnd`).

  **Solución para `'enabled'`** (pendiente — ~1.5 días, se hará más adelante). Todo se agrega a `wireRouterScroller` sin tocar `anchorScrolling`/`'top'`:
  1. **Detectar back/forward**: en el `.run`, `platformLocation.onPopState(() => { pendingPop = true; })` (ya existe `PlatformLocation.onPopState`). El próximo `$transitions.onSuccess` lee y limpia `pendingPop`; limpiarlo también en `onError`/`onCancel`. (`transition.options().source` no distingue "URL tipeada" de "botón atrás", por eso hay que escuchar `window` directo.)
  2. **Store de posiciones** — elegir UNA:
     - **Por URL** (recomendado, simple): `Map<string, [number, number]>`. En `$transitions.onBefore` guardar `store.set($location.url(), viewportScroller.getScrollPosition())` (posición de la página que se deja). En `onSuccess`, si `pendingPop` y hay `store.get(nuevaUrl)` → `scrollToPosition(pos)` (diferido). Matiz vs Angular: también restaura al volver a una URL ya visitada por un **link** (no solo con el botón atrás) — aceptable.
     - **Por `navigationId`** (fiel, frágil): estampar `history.replaceState({ ...history.state, ngjsNavId: n }, ...)` después de cada cambio de URL de `$location`; leer `history.state.ngjsNavId` en el popstate para keyear el store. Riesgo: `$location` en html5Mode puede pisar `history.state`.
  3. **Prioridad en `onSuccess`**: `pos` restaurado (si `pendingPop`) → `scrollToAnchor` (si `anchorScrolling` y hay fragment) → `scrollToPosition([0,0])` (si `'top'`/`'enabled'` forward).
  4. **Evento `Scroll`** (opcional, fiel): clase en `src/router/events.ts` (`{ routerEvent: NavigationEnd, position, anchor }`), emitida por `RouterImpl` (exponer un `ɵemitScroll` o mover el wiring del scroller adentro de `RouterImpl`). Casi nadie lo consume.
