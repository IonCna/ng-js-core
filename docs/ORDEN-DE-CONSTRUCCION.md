...
Subpaths: `ngjs-core/platform-browser` (clases con forma Angular) y `ngjs-core/runtime/platform-browser` (`PlatformBrowserModule` / `platformBrowserModule()` / `providePlatformBrowser()` — `angular.module` opt-in, dep `ng.js.core`). `@angular/cdk` va aparte en `ngjs-core/cdk/{a11y,layout}`.
